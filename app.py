...
app = Flask(__name__, template_folder="../frontend")  # Points to frontend/ folder
CORS(app)
...
def map_parameters(scenario, data):
    ...
    mapped = {}
    for front_param, back_param in param_mapping[scenario].items():
        if front_param in data:
            value = data[front_param]
            # Special conversions
            if front_param == 'frequency_mhz' or front_param == 'frequency_mhz_cellular':
                value = value / 1000  # Convert MHz to GHz
            elif front_param == 'base_station_power_w':
                value = 10 * math.log10(value * 1000)  # Convert W to dBm
            elif front_param == 'coverage_area_km2':
                value = math.sqrt(value / math.pi)  # Convert area to radius
            elif front_param == 'user_density_per_km2':
                area = data.get('coverage_area_km2', 1)
                value = value * area  # Calculate total users
            mapped[back_param] = value
    return mapped

@app.route('/api/wireless-communication', methods=['POST'])
def wireless_communication():
    return handle_calculation('wireless_comm')
...
def handle_calculation(scenario):
    ...
    parameters = map_parameters(scenario, data)
    ...
    try:
        ...
        return jsonify({"success": True, "results": results, "explanation": explanation})
